...
Start monitor mode(wof)[1]       
Stop monitor mode (wof)[2]                      
Scan Networks (wof)[3]                            
Getting Handshake(wof)[4]   
Install Wireless tools & Wolf Linux tools(wof)[5]                   
Crack Handshake with wordlists(wof)[6] 
Crack Handshake with wordlist (wof)[7] 
Crack Handshake without wordlist(wof) [8] 
Create wordlist(wof)[9]                                     
WPS Networks attacks(wof)[10] 
Scan for WPS Networks(wof)[11] 
About key(wof)[0]
Exit key(wof)00]
-----------------------------------------------------------------------
""")
...
""")
        ...
        cmd = os.system("clear")
        print("""
 The Only for Running for Linux Kernel And Linux Os ,Wolf Software Running for Tools 
...
""")
        ...
        if attack == 1:
            ...
            intro() #The value for not consider the attack they wifi attack against
    elif var == 11:
        print("\nEnter the interface to start ?(Default(Wlan0mon/Wlan1mon)")
        interface = str(input(""))
